...
function addOwnerDataToMap(dataLayer, choice) {
    var url = 'https://eichnersara.cartodb.com/api/v2/sql?' + $.param({
       q: 'SELECT * FROM bk_oddlots ORDER BY ownertype DESC LIMIT ' + choice,
       format: 'GeoJSON'
    });
    $.getJSON(url)
  ...
  // When it's done, add the results to the map
    ...
    dataLayer.addOwnerDataToMap(data);
    });
    ...
    $(document).ready(function () {
        ...
        //addTileLayer(map);
        addOwnerDataToMap(dataLayer, 10);

        $('.choice').change(function () {
        addOwnerDataToMap(dataLayer, $(this).val());
